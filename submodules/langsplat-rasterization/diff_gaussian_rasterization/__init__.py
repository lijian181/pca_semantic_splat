...
# diff_gaussian_rasterization/__init__.py 文件
# 替换为这个最终版本的 _RasterizeGaussians 类
...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        # --- 关键修正点 ---
        # 在将参数传递给C++之前，进行安全检查
        # 将 Python 的 None 转换为空的 torch.Tensor
        if sh is None:
            sh = torch.Tensor([])
        ...
        # C++前向函数现在会返回7个值，我们需要为深度图的返回值准备一个接收变量
    ...
    def backward(ctx, grad_out_color, grad_out_language_feature, _):
        ...
        colors_precomp, language_feature_precomp, means3D, scales, rotations, cov3Ds_precomp, radii, sh, opacities, geomBuffer, binningBuffer, imgBuffer = ctx.saved_tensors
        ...
        grad_out_depth = torch.Tensor([]).to(grad_out_color.device)
