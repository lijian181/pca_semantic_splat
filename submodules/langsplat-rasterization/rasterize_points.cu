    ...
    const bool include_feature)
{
  ...
  float* dL_dinvdepthsptr = dL_dinvdepths.data<float>();
  float* dL_dout_invdepthptr = (dL_dout_invdepth.numel() > 0) ? dL_dout_invdepth.contiguous().data<float>() : nullptr;
